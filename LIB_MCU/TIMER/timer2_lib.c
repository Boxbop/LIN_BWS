...
void Timer2_start(U16 timout,U8 dlc){
  TIMSK2  = 1 << TOIE2;    // Int T2 Overflow Enabled
...
#error "timer 2 prescaler not defined"
  ...
  TCCR2A  = (1<<CS22)  | (1<<CS21);         // Set clock source to fosc / 256
#else
...
void Timer2_stop(void){
  TIMSK2  &= ~(1 << TOIE2);    // disable Int T2 Overflow Enabled
#ifdef AT90CAN128_USED
  TCCR2A  = 0x00; //stop timer clock
#else
